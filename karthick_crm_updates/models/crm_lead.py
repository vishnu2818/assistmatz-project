...
class Lead(models.Model):
    ...
    def write(self, vals):
        """
        Full write with:
         - auto-move New -> Quote Preparation (fixed to work for pipeline)
         - manual stage validations
         - safe recursion bypassing
        """
        ...
        # If bypass flag used → skip validations
        if self.env.context.get('bypass_stage_validations'):
            return super(Lead, self).write(vals)
    
        # ----------------- ORIGINAL AUTO MOVE (pre-write) -----------------
        ...
        # ----------------- MANUAL STAGE CHANGE VALIDATIONS -----------------
        for lead in self:
        ...
        # ----------------- FIXED AUTO-MOVE (POST-WRITE) -----------------
        # This is the ONLY new change. Ensures pipeline creation works.
        if not self.env.context.get('skip_auto_stage'):
            for lead in self:
                try:
                    # If still in New → and mandatory fields complete → move to Quote Preparation
                    if lead.stage_id and lead.stage_id.name == 'New':
                        missing = lead._check_required_fields_on_stage_change()
                        if not missing:
                            next_stage = LeadStage.search(
                                [('name', '=', 'Quote Preparation')], limit=1)
    
                            if next_stage and lead.stage_id.id != next_stage.id:
                                lead.with_context(
                                    skip_auto_stage=True,
                                    bypass_stage_validations=True
                                ).write({'stage_id': next_stage.id})
                                _logger.info(
                                    "AUTO: Lead %s NEW → QUOTE PREPARATION (post-write)", lead.id)
    
                except Exception as e:
                    _logger.exception("AUTO UPDATE FAILED (post-write): %s", e)
